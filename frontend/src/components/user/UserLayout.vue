            ...
            <div class="relative" ref="menuRef">
              ...
              >
                ...
                <div class="hidden md:block text-left mr-1">
                   <p class="text-xs font-black tracking-tight text-gray-900 dark:text-white">{{ authStore.isAuthenticated ? (authStore.user?.name || 'Petani') : 'Yuk Login' }}</p>
                   <p class="text-[9px] text-epanen-primary dark:text-epanen-accent opacity-80 uppercase font-black tracking-widest leading-none mt-0.5">{{ authStore.isAuthenticated ? 'Mitra Tani' : 'Klik Disini' }}</p>
                </div>
              ...
              >
                <div v-if="authStore.isAuthenticated" class="px-5 py-4 border-b border-gray-50 dark:border-white/5 bg-gray-50/50 dark:bg-white/5 mb-2">
                ...
                <div v-else class="px-5 py-6 text-center">
                  <p class="text-sm font-black text-gray-900 dark:text-white mb-4">Mari bergabung dengan ePanen!</p>
                  <router-link to="/login" class="block w-full py-2 bg-epanen-primary text-white rounded-xl text-xs font-black transition-transform hover:scale-105" @click="showMenu = false">
                    MASUK SEKARANG
                  </router-link>
                </div>
        ...
        <div :class="['grid grid-cols-1 md:grid-cols-4 gap-24 border-b pb-20 mb-16', themeStore.isDarkMode ? 'border-white/10' : 'border-gray-100']">
          ...
          <div class="space-y-10">
            ...
            <nav class="flex flex-col space-y-8 font-bold text-emerald-50/60 text-sm">
               <router-link to="/chat" class="hover:text-epanen-accent transition-colors">Kian AI Assistant</router-link>
...
const navLinks = [
  ...
  { to: '/chat', label: 'Kian AI', iconSvg: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>' },
